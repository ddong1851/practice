...
public class Main_백준_17135_캐슬디펜스_골드4_김동률_236ms {
	...
	public static void main(String[] args) throws Exception{
		...
		/*
		 * 문제 조건 입력 행의 수 N, 열의 수 M, 궁수의 공격 사거리 D 3 <= N, M <= 15 1 <= D <= 10 --> 완탐이라
		 * 데이터가 적은 것 같다.
		 * 
		 * IDEA 
		 * 궁수의 배치는 완전 탐색으로 한다. 궁수들이 동일한 적을 공격할 수 있으므로 공격당한 적들의 위치를 따로 저장하고, 
		 * 적군이 전진하기 이전에 게임에서 제외시킨다. 
		 * 거리 계산은 가독성을 위해 메서드로 구현 + 좌표 이동
		 * 적군 찾기는 궁수의 좌표 기준으로
		 * BFS로(왼쪽 우선 가까운 적)
		 * + 제외된 녀석들을 0으로 갱신 + 갱신된 좌표의 수 -> 처치한 적 + 1턴 끝나면 적군 전진 -> 성벽에 제외
		 * 
		 * 적 위치 배열 [(1, 2).....] or List
		 * 궁수 1 기준 적군의 거리 저장 [(1,5), (2,4), (3,3), (4,8), ..] 
		 * 궁수 2 기준 적군의 거리 저장 [(1,5), (2,4), (3,3), (4,8), ..] 
		 * 궁수 3 기준 적군의 거리 저장 [(1,5), (2,4), (3,3), (4,8), ..] 
		 * --> 거리 순으로 정렬
		 * 사거리 안에 죽은 적들 처리 ( 큐에 넣기 )
		 * 한턴이 끝나면 적군의 거리를 1씩 감소 ( 0이 되면 없앤다 ) or 거리 - days < = 0 이면 다음
		 */
		...
		// 궁수의 좌표에 따른 완전 탐색 ( 배치를 0,1,2 -> 0,1,3 이런식으로 반복하여 처리한 적의 최데 수를 계산한다. )
		for(int i=1; i<=M-2; i++) {
