...
public class Main_백준_5639_이진검색트리_골드5_444ms {
	...
	public static void main(String[] args) throws Exception{
		...
		/* 
		 * 이진 탐색 트리란,
		 * 정렬된 이진 트리로, 노드 기준 왼쪽에 위치한 노드들은 기준 노드보다 작고, 우측에 위치한 노드들은 기준 노드보다 크다
		 * 서브 트리들 또한 이진 탐색 트리로 구성되어 있으며
		 * 중복된 키를 허용하지 않습니다.
		 * 장점
		 * 편향된 이진 트리가 아닐 경우 특정 값을 찾는데 소요되는 시간이 O(logN) ( 이진 탐색과 동일 ) 
		 * 구현부를 보면 연결 리스트와 유사 -> 연결 리스트의 장점인 빠른 삽입 / 삭제를 가능하게 하고, 이진 검색의 장점도 갖는다.
		 * 
		 * 단점 
		 * 편향 트리로 구성된다면 구현에 의미가 없다 ( 검색이 O(n) ) 
		 * ( 하지만 이를 보완한 레드 블랙 트리가 있다, Rotate 메서드를 구현하여 편향되지 않게 ) 
		 * 
		 * 문제 접근
