...
public class Solution_카드짝맞추기 {
	...
	public int solution(int[][] board, int r, int c) {
		...
		/*
		 * BFS, DFS 모두 가능할 것 같다. Enter 를 클릭해서 카드를 뒤집는데, 필요없는 짝을 들추면 안되니까 카드를 뽑는 순서를 먼저
		 * 결정하고 해당 카드에 도착했을 때만 Enter 를 클릭해서 뒤집는다.
		 * 
		 * 방문 처리는 어떤 카드를 발견하면 초기화 해야한다.
		 * 
		 * --> 순열? 을 통해 카드를 뽑는 순서를 결정하고, BFS 로 각각을 뒤집는 최소 수를 카운팅하고, 최소 값을 넘길 경우 break
		 * 하자. (4x4니까 굳이 하지 않아도 될듯)
		 * 
		 */
